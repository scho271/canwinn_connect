...
//                         ],
//                       ),
//                       Obx(() {if (controller.categoriesController.isLoading.value) {
...
//       case 11:
//         return HospitalNearMeWidget(categoryId: category.id, categoryName: category.name);
//       case 12:
//         return DoctorAppointmentScreenWidgets(categoryId: category.id, categoryName: category.name);
//       case 13:
//         Future.microtask(() => showApplianceRepairBottomSheet(category)); // safe
//         return Container();
//       case 14:
...
//
//
//   showApplianceRepairBottomSheet(dynamic category) {
...
//                 } else if (controller.categoriesController.categoriesList.isEmpty) {
//                   return const Expanded(child: Center(child: Text("No categories found")));
//                 } else {
//                   final applianceCategory = controller.categoriesController.categoriesList
//                       .firstWhere((cat) => cat.name == "Appliance Repair");
//
//                   if (applianceCategory.subcategories!.isEmpty) {
//                     return const Expanded(child: Center(child: Text("No subcategories found")));
//                   }
//
//                   return Expanded(
...
//                       ),
//                       itemCount: applianceCategory.subcategories?.length,
//                       itemBuilder: (context, index) {
...
//                                           ? Image.network(
//                                         AppApiUrl.imagebaseUrl + subcategory!.image.toString(),
//                                         fit: BoxFit.cover,
...
class _DashboardState extends State<Dashboard> {
  ...
  Widget getCategoryContent(dynamic category) {
    switch (category.id) {
      ...
      case 12:
        return DoctorAppointmentScreenWidgets(categoryId: category.id, categoryName: category.name);
      case 13:
      // Display category name and open bottom sheet on tap
        return GestureDetector(
          onTap: () {
            showApplianceRepairBottomSheet(category);
          },
          child: Container(
  ...
  void showApplianceRepairBottomSheet(dynamic category) {
    final controller = Get.find<DashboardController>();
